  ...
  useEffect(() => {
    const fetchBackgrounds = async () => {
      try {
        ...
        // Add any classic backgrounds if they exist
        if (response.data.classic?.backgrounds?.length > 0) {
          transformedCategories.classic = {
            title: 'Classic',
            subtitle: 'Timeless Marathon Moments',
            description: 'Traditional marathon experience',
            icon: 'ðŸ…',
            color: '#6B7280',
            backgrounds: response.data.classic.backgrounds
          };
        }
